...
(defvar-local eaf-ebook-viewer-document-text nil
  "Latest visible-page text and document information from Ebook Viewer.")
...
(defvar-local eaf-ebook-viewer-text-highlights nil
  "Text-highlight entries currently supplied by an external integration.")
(defvar-local eaf-ebook-viewer-text-highlight-status nil
  "Latest text-highlight rendering status reported by the book frame.")
(defvar eaf-ebook-viewer-annotation-created-hook nil
  "Hook run when Calibre creates a native highlight.
Each function receives one annotation event alist.")
(defvar eaf-ebook-viewer-annotation-removed-hook nil
  "Hook run when Calibre removes a native highlight.
Each function receives one annotation event alist.")
(defvar eaf-ebook-viewer-annotation-updated-hook nil
  "Hook run when Calibre updates a native highlight.
Each function receives one annotation event alist.")
(defvar eaf-ebook-viewer-annotation-clicked-hook nil
  "Hook run when a native Calibre highlight is selected.
Each function receives one annotation event alist.")
(defvar eaf-ebook-viewer-book-loaded-hook nil
  "Hook run after Calibre metadata and manifest become available.")
(defvar eaf-ebook-viewer-document-changed-hook nil
  "Hook run after Ebook Viewer reports different visible-page text.")
(defvar eaf-ebook-viewer-word-clicked-hook nil
  "Hook run after a word is clicked in Ebook Viewer.
Each function receives one text-context alist.")
...
(defun eaf-ebook-viewer-set-book-data (buffer-id metadata manifest)
  ...
  (when-let ((buffer (eaf-get-buffer buffer-id)))
    (with-current-buffer buffer
      ...
      (run-hooks 'eaf-ebook-viewer-book-loaded-hook))))

(defun eaf-ebook-viewer-set-document-text (buffer-id payload)
  "Store visible-page text PAYLOAD for Ebook Viewer BUFFER-ID."
  (when-let ((buffer (eaf-get-buffer buffer-id)))
    (with-current-buffer buffer
      (setq eaf-ebook-viewer-document-text
            (eaf-ebook-viewer--parse-json payload))
      (run-hooks 'eaf-ebook-viewer-document-changed-hook))))

(defun eaf-ebook-viewer-notify-annotation-created (buffer-id payload)
  "Notify BUFFER-ID that Calibre created the annotation in PAYLOAD."
  (when-let ((buffer (eaf-get-buffer buffer-id)))
    (with-current-buffer buffer
      (run-hook-with-args 'eaf-ebook-viewer-annotation-created-hook
                          (eaf-ebook-viewer--parse-json payload)))))

(defun eaf-ebook-viewer-notify-annotation-removed (buffer-id payload)
  "Notify BUFFER-ID that Calibre removed the annotation in PAYLOAD."
  (when-let ((buffer (eaf-get-buffer buffer-id)))
    (with-current-buffer buffer
      (run-hook-with-args 'eaf-ebook-viewer-annotation-removed-hook
                          (eaf-ebook-viewer--parse-json payload)))))

(defun eaf-ebook-viewer-notify-annotation-updated (buffer-id payload)
  "Notify BUFFER-ID that Calibre updated the annotation in PAYLOAD."
  (when-let ((buffer (eaf-get-buffer buffer-id)))
    (with-current-buffer buffer
      (run-hook-with-args 'eaf-ebook-viewer-annotation-updated-hook
                          (eaf-ebook-viewer--parse-json payload)))))

(defun eaf-ebook-viewer-notify-annotation-clicked (buffer-id payload)
  "Notify BUFFER-ID that Calibre selected the annotation in PAYLOAD."
  (when-let ((buffer (eaf-get-buffer buffer-id)))
    (with-current-buffer buffer
      (run-hook-with-args 'eaf-ebook-viewer-annotation-clicked-hook
                          (eaf-ebook-viewer--parse-json payload)))))
...
(defun eaf-ebook-viewer-set-text-highlight-status (buffer-id payload)
  "Store text-highlight rendering status for Ebook Viewer BUFFER-ID."
  (when-let ((buffer (eaf-get-buffer buffer-id)))
    (with-current-buffer buffer
      (setq eaf-ebook-viewer-text-highlight-status
            (eaf-ebook-viewer--parse-json payload)))))

(defun eaf-ebook-viewer-set-text-highlights (words)
  "Highlight WORDS without modifying book text or native annotations.
Each entry may be a string or an alist containing `word' and `style'."
  (setq eaf-ebook-viewer-text-highlights
        (delete-dups
         (seq-filter (lambda (entry)
                       (or (stringp entry)
                           (and (listp entry)
                                (stringp (alist-get 'word entry)))))
                     words)))
                  ...
                  (concat
                   ...
                   (base64-encode-string
                    (encode-coding-string
                     (json-serialize
                      (vconcat eaf-ebook-viewer-text-highlights))
                     'utf-8)
                    t))))

(defun eaf-ebook-viewer-clear-text-highlights ()
  "Clear externally supplied text highlights in the current book."
  (setq eaf-ebook-viewer-text-highlights nil)
  (eaf-call-async "execute_function"
                  eaf--buffer-id
                  "clear_text_highlights"))

(defun eaf-ebook-viewer-create-highlight ()
  "Create a native Calibre highlight from the current selection."
  (interactive)
  (eaf-call-async "eval_function"
                  eaf--buffer-id
                  "create_highlight"
                  (key-description (this-command-keys-vector))))

(defun eaf-ebook-viewer-delete-highlight (uuid)
  "Delete the native Calibre highlight identified by UUID."
  (eaf-call-async "execute_function_with_args"
                  eaf--buffer-id
                  "delete_highlight"
                  uuid))

(defun eaf-ebook-viewer-set-highlight-notes (uuid notes)
  "Set plain-text NOTES on the native Calibre highlight UUID."
  (eaf-call-async "execute_function_with_args"
                  eaf--buffer-id
                  "set_highlight_notes"
                  uuid notes))
